...
Deseja-se aproximar uma função desconhecida, $f(x)$, que relaciona $y$ e $x$. Tal aproximação pode ser feita considerando uma função perda, $L(y,f(x))$, que mede os erros de previsão.
...
$$
E[(y-\hat f(x))^2] = Var[\hat f(x)] + Vicio[\hat f(x)] + Var[\varepsilon]
$$
...
$$y_{10} = \beta_0 + \beta_1x + \beta_2x^2 + \beta_3x^3 + \beta_4x^4 + ... + \beta_{10}x^{10} $$
