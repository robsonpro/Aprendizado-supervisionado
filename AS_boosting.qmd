...
boost1 <- gbm(wage~., data = dados, distribution = "gaussian", 
              n.trees = 8000, bag.fraction = 0.7, interaction.depth = 11, shrinkage = 0.2)
# summary(boost1)
```

Seja um conjunto de dados para prever o rendimento de indivíduos considerando anos de experiência, ocupação, se trabalha ou não na indústria, escolaridade, além de fatores sócio-geográficos. Considerando um modelo obtido via GBM 8000 árvores e $\lambda = 0.2$, pode-se plotar um gráfico de superfície considerando como variáveis independentes escolaridade  anos de experiência conforme segue.   
