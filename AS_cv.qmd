...
A validação cruzada via $k$-dobras ($k$-fold cross-validation) é a abordagem mais usada dadas suas propriedades e simplicidade. Particionando os dados disponíveis em $K$ folds, $k = 1, ..., K$, com aproximadamente mesmo número de observações em cada partição, $n_k \sim N/K$, para a $k$-ésima partição o modelo é estimado usando os dados das outras $K-1$ partições ou dobras, e o erro é estimado na $k$-ésima dobra. Seja a função aproximada com a $k$-ésima dobra excluída, $\hat{f}^{-k(i)}(\mathbf{x}_i,\alpha)$. O erro de generalização ou teste é calculado via validação cruzada segundo a Equação a seguir. Mais detalhes podem ser encontrados em Hastie et al. (2009).
