...
Seja a modelagem de uma resposta $y$ a qual supõe-se dependente de um vetor de variáveis de entrada ou preditoras $\mathbf{x} = [x_1, ..., x_k]^T$.
Um modelo desconhecido pode ser denotado $f(\mathbf{\mathbf{x}, \mathbf{\alpha}})$, onde $\mathbf{\alpha}$ consiste em um (ou um vetor de) hyperparâmetro que minimiza o erro de previsão.
Um conjunto de observações de treino, $\mathcal{T} = (\mathbf{x}_1,y_1), ..., (\mathbf{x}_N,y_N)$, obtidos da distribuição de probabilidade conjunta de $\mathbf{x},y$, $P(\mathbf{x},y)$, está disponível para o propósito de estimativas. 
O modelo estimado, $\hat{f}(\mathbf{x}, \mathbf{\alpha})$, é obtido pela minimização de uma função perda, que mede a distância entre o modelo e as observações de treino.
No caso de problemas de regressão, $y \in \mathcal{R}^1$,a escolha mais comum é a função perda quadrática, $L(y,\hat{f}(\mathbf{x}, \mathbf{\alpha})) = (y-\hat{f}(\mathbf{x}, \mathbf{\alpha}))^2$. 
Em casos de problemas de classificação binária, $y \in [0,1]$, quando deseja-se modelar probabilidades de pertencer a uma das classes, $p(\mathbf{x}) = Pr(y|\mathbf{x})$, a função perda de entropia cruzada é a escolha mais comum, $L(y,\hat{h}(\mathbf{x}, \mathbf{\alpha})) = -2(y log(p(\mathbf{x}))+(1-y)log(1-p(\mathbf{x})))$.
O erro de generalização ou de teste pode ser calculado segundo a Equação a seguir, onde $(\mathbf{x}_0,y_0)$ é uma nova observação.
$$
    Err_\mathcal{T} = E[L(y_0,\hat{f}(\mathbf{x}_0, \mathbf{\alpha}))|\mathcal{T}]
$$
O erro de treino, $\overline{err} =  (1/N)\sum_{i=1}^{N}L(y,\hat{h}(\mathbf{x}, \mathbf{\alpha}))$, sempre será maior que o erro de generalização, $Err_\mathcal{T}$.
O erro esperado de generalização pode ser calculado como $Err = E[L(y_0,\hat{f}(\mathbf{x}_0, \mathbf{\alpha}))] = E[Err_\mathcal{T}]$. A forma mais eficiente e prática de estimar o erro esperado de previsão é através da validação cruzada (HASTIE et al., 2009).

### Validação cruzada por dados de treino e teste

A validação cruzada por dados de treino e teste é basicamente o que foi feito nos métodos aprendidos até aqui. As $N$ observações de treino disponíveis, $\mathcal{T}$, são divididas em observações de treino e teste. O modelo é estimado usando as observações de treino e testado nas observações de teste. Conforme explicado, a própria função perda pode ser usada para avaliar a capacidade de generalização do modelo, mas, na prática, algumas outras métricas podem ser úteis para avaliar o erro em dados de teste que visa estimar o erro do modelo em dados futuros ou sua capacidade de generalização.

A métrica $R^2$, $R^2 \in [0,1]$, denominada coeficiente de determinação, consiste na proporção da variabilidade dos dados explicados pelo modelo.
Quanto maior é o valor de $R^2$ para os dados de teste, maior a capacidade de generalização do modelo.
$$
...
O erro médio absoluto (*mean absolute error* -  MAE) mede a média dos valores absolutos das diferenças entre o valor real e o valor predito. Essa métrica é mais robusta a \textit{outliers}. 
$$
...
A validação cruzada via $k$-dobras (*$k$-fold cross-validation*) é a abordagem mais usada dadas suas propriedades e simplicidade.
Particionando os dados disponíveis em $K$ folds, $k = 1, ..., K$, com aproximadamente  mesmo número de observações em cada partição, $n_k \sim N/K$, para a $k$-ésima partição o modelo é estimado usando os dados das outras $K-1$ partições ou dobras, e o erro é estimado na $k$-ésima dobra. 
Seja a função aproximada com a $k$-ésima dobra excluída, $\hat{f}^{-k(i)}(\mathbf{x}_i,\alpha)$.
O erro de generalização ou teste é calculado via validação cruzada segundo a Equação a seguir.
Mais detalhes podem ser encontrados em Hastie et al. (2009).
...
A maior vantagem do $k$-dobras para o LOO é o tempo compuational. Tomando $k=10$, por exemplo, serão estimados 10 modelos, enquanto a validação via LOO estimará $N$, o que pode implicar em alto tempo compuational para tamanhos amostrais grandes, especialmente para métodos de aprendizado mais complexos (GARETH et al., 2013).
...
Tomando o conjunto de dados, inicialmente faz-se o sorteio das observações de cada partição. Neste caso, para $N=30$ e tomando $K=10$ dobras, tem-se exatamente $n_k=30/10=3$. A seguir todo  conjunto de dados é exposto considerando o particionamento realizado de forma aleatória.
...
Observação: A escolha de hiperparâmetros em alguns métodos, sobretudo os de origem estatística, pode ser baseada também em critérios de informação, como o AIC. Entretanto, neste curso, dar-se-á ênfase em validação cruzada uma vez que está técnica é útil tanto para os métodos de origem na estatística quanto para os de origem na computação, além de ser simples de implementar.
