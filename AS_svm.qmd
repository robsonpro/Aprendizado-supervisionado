...
Seja um problema de classificação binário, $y=\{-1,1\}$ em função de duas variáveis regressores, $\mathbf{x}=[x_1,x_2]^T$, conforme ilustrado abaixo.
...
A regra de classificação usando um hiperplano que faz a classificação perfeita é descrita a seguir:
$$
\begin{align}
...
ggplot(data, aes(x = x1, y = x2, color = classe, pch = classe)) +
  ...
  theme_bw()
```
Logo, para casos perfeitamente separáveis não sequer um ponto que satisfaça $b+w_1x_1+w_2x_2=0$, porém todos os pontos satisfazem a seguinte condição:
$$
\begin{align}
...
Tal problema é análogo à seguinte formulação. O quadrado da norma facilita a otimização, transformando a função a ser minimizada em convexa.
$$
\begin{aligned}
...
Substituindo tais resultados na função lagrandgeana, pode-se formular o problema dual, conforme segue.
$$
\begin{aligned}
    \text{Max } & \begin{Bmatrix}
...
Obviamente é importante considerar o kernel já na otimização, de forma que os hiperparâmetros específicos destes também sejam selecionados adequadamente aos dados. Para o exemplo plotado anteriormente, utilizando um kernel polinomial e metade das observações para treino, obtém-se o classificador plotado a seguir com os dados remanescentes.
