...
Utilizando o modelo para previsão com dados de teste (dados futuros).
...
```{r}
pred.te <- predict(lm1, newdata=Consumo.te)
...
Pacote para dados e para codificação dummy.
...
#| message: false
#| warning: false
ggpairs(Grunfeld[1:80,], columns = 1:4, aes(color = firm)) + theme_bw()
```
...
```{r}
Grunfeld <- dummy_cols(Grunfeld, select_columns = c("firm"))

Grunfeld <- Grunfeld[,-c(4,16)] # removendo coluna da variável "firm" e coluna da última variável binária criada, a qual é desnecessária
...
```{r}
Grunfeld[,2:4] <- scale(Grunfeld[,2:4])
```
...
```{r}
set.seed(87)
tr <- round(0.8*nrow(Grunfeld),0)
treino <- sample(nrow(Grunfeld), tr, replace = F)
Grunfeld.tr <- Grunfeld[treino,]
Grunfeld.te <- Grunfeld[-treino,]
```
